...
for i in range(len(results)):
	...
	plt.plot(x, S_plot, label=f"kond S(x, t={t:.1f})", color='blue')
	plt.plot(x, Seq_plot, label=f"buck S_eq(x, t={t:.1f})", color='green')
	plt.plot(x, z_plot, label=f"z(x, t={t:.1f})", color='red')
	plt.xlabel("x")
	plt.ylabel("S(x), z(x)")
	plt.title(f"Solution at t = {t:.1f}, mu1/mu2 = {mu1/mu2:.3f}, tau={tau:.2f}")
	plt.xlim(0, 1)
	plt.ylim(0, 1)
	plt.grid()
	plt.legend()
